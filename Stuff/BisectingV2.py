...
class BisectingSVM:
    ...
    def run_bisecting_svm(self, klaster_sets, c_values, gamma_values):
        ...
        for i, klaster_set in enumerate(klaster_sets):
            ...
            for key, cluster in enumerate(raw_clusters):
                for tupel in cluster:
                    data.append(bisecting.converting_ecf(tupel))
                    actual_y.append(key)  # Assign cluster label

            # Optimize SVM for classification
            svm_model, _ = self.optimize_svm(data, actual_y, c_values, gamma_values)

            # Predict cluster label for test data
            predicted_cluster = svm_model.predict([test_data])[0]
            errors.append(abs(predicted_cluster - raw_test_data['actualPF']))
